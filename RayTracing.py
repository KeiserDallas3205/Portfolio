...
# Date: 2/21/2023
# CSC 470-001
# Assignment 4: This assignment contains a scene with:
#                   - Phong Illuminated Checkerboard w/ reflective properties
#                   - Three highly reflective Phong Illuminated spheres (red, green, and blue)
#                   - A blue sky
#               The scene is painted pixel by pixel through Phong Illumination and Ray Tracing. 
######################################
...
#********************* Constants **************************************************
centerOfProjection = [0,0,-500] # Viewpoint
Ia = .3
Ip = .7
L = [1,1,-1] # Point light source
lightSource = [500,500,0] # Light source used for shadows
V = [0,0,1]
illuminationSaturationCounter = 0
skyBoxColor = [0.53,0.81,0.92] # Blue
horizon = 1500 # Depth of checkerboard
...
def traceRay(startPoint,ray,depth):
    ...
    if(nearest == []):
        ...
        # Board
            # Get pixel intensity 
            intensity = boardInt(intersection,nearestObject)
            ...
            r = reflect([0,1,0],ray)
            reflectedColor = traceRay(intersection,r,depth-1)
            reflectedWeight = 0.5

            # Combine the color
            combineColor = [(localColor[i]* localWeight) + (reflectedColor[i] * reflectedWeight) for i in range(len(reflectedColor))]

            return combineColor
        else:
        # Sphere
            # Calculate normal at that point
            n = [intersection[i] - nearestObject[0][i] for i in range(len(intersection))]
            
            # Get pixel intensity
            intensity = sphereInt(n,nearestObject)
...
def boardInt(point,object):
    ...
    # Boolean checks if the pixel is out of bounds of the picture
    outOfBounds = False
...
# This function checks if an object is within shadow
# Returns a boolean 'dim' if the object is in shadow
def inShadow(startObject,startPoint):
    dim = False
    ray = vector(startPoint,lightSource)
    # Check and see if the object intersects with any other object in the scene
    # If so, dim the pixel
    for object in scene:
...
def RGBColorHexCode(color):
    ...
    for intensity in color:
        ...
        # Add the new portion to the current color string
